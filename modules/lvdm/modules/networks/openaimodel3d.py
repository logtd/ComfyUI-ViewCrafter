    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None, padding=1, dtype=None, device=None, operations=ops):
        ...
        if use_conv:
            self.op = operations.conv_nd(
                dims, self.channels, self.out_channels, 3, stride=stride, padding=padding, device=device, dtype=dtype
            )
        else:
...
class UNetModel(nn.Module):
            ...
            for i in range(num_res_blocks + 1):
                ...
                if level and i == num_res_blocks:
                    ...
                    layers.append(
                        ...
                        if resblock_updown
                        else Upsample(ch, conv_resample, dims=dims, out_channels=out_ch, device=device, dtype=dtype)
                    )
                    ds //= 2
    ...
    def forward(
        ...
        for id, module in enumerate(self.input_blocks):
            ...
            #h = module(h, emb, context=context, batch_size=batch_size)
